...
caracteristicas_pd = caracteristicas.to_pandas()
...
# --- 1. K-MEANS ---
res_km = clustering.clustering_kmeans(X, ids)
est_km = calidad_clustering.obtener_estabilidad(X, clustering.clustering_kmeans, ids, res_km["parametros"])

comparativa.append({
    "Modelo": "K-Means",
    "Silhouette": calidad_clustering.obtener_silhouette(X, res_km["labels"]),
    "Davies-Bouldin": calidad_clustering.obtener_davies_bouldin(X, res_km["labels"]),
    "Estabilidad": est_km
})

# --- 2. DBSCAN ---
res_db = clustering.clustering_dbscan(X, ids)

comparativa.append({
    "Modelo": "DBSCAN",
    "Silhouette": calidad_clustering.obtener_silhouette(X, res_db["labels"]),
    "Davies-Bouldin": calidad_clustering.obtener_davies_bouldin(X, res_db["labels"]),
    "Estabilidad": 0.0
})

# --- 3. JERÁRQUICO ---
res_jer = clustering.clustering_jerarquico(X, ids)
est_jer = calidad_clustering.obtener_estabilidad(X, clustering.clustering_jerarquico, ids, res_jer["parametros"])

comparativa.append({
    "Modelo": "Jerárquico",
    "Silhouette": calidad_clustering.obtener_silhouette(X, res_jer["labels"]),
    "Davies-Bouldin": calidad_clustering.obtener_davies_bouldin(X, res_jer["labels"]),
    "Estabilidad": est_jer
})

# --- TABLA FINAL ---
print("\n" + "="*70)
print(f"{'ALGORITMO':<15} | {'SILHOUETTE':<12} | {'D-BOULDIN':<12} | {'ESTABILIDAD':<12}")
print("-"*70)
for m in comparativa:
    print(f"{m['Modelo']:<15} | {m['Silhouette']:<12.4f} | {m['Davies-Bouldin']:<12.4f} | {m['Estabilidad']:<12.4f}")
print("="*70)
