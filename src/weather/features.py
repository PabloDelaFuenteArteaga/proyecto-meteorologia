...
def caracteristicas_climaticas(df: "pl.DataFrame") -> "pl.DataFrame":
    ...
    df = df.with_columns(pl.col("date").dt.month().alias("mes"))
    ...
    # Estacionalidad
    mensual = df.group_by(["station_id", "mes"]).agg(
        pl.col("temp").mean().alias("temp_media_mes")
    )

    mensual_pivot = mensual.pivot(
        values="temp_media_mes", index="station_id", columns="mes"
    )
